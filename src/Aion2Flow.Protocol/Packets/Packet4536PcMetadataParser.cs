...
internal readonly record struct Packet4536PcMetadata(int EntityId, string Nickname, int NicknameLength, int TailOffset, int ClassCode, byte FactionCode);
...
internal static class Packet4536PcMetadataParser
{
    ...
    public static bool TryParsePayload(ReadOnlySpan<byte> payload, out Packet4536PcMetadata result)
    {
        ...
        for (var markerOffset = searchStart; markerOffset < searchEnd; markerOffset++)
        {
            ...
            result = new Packet4536PcMetadata(entityId, sanitizedName, nicknameLength, tailOffset, code, factionCode);
            return true;
        }
