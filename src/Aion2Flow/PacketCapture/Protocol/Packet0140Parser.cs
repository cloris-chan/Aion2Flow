...
internal readonly record struct Packet0140State(
    uint Value0,
    ushort Value1,
    int TailLength,
    string TailSignature);

internal static class Packet0140Parser
{
    public static bool TryParse(ReadOnlySpan<byte> packet, out Packet0140State result)
    {
        result = default;
        ...
        if (packet[reader.Offset] != 0x01 || packet[reader.Offset + 1] != 0x40) return false;
        reader.TryAdvance(2);
        ...
        result = new Packet0140State(value0, value1, tailLength, tailSignature);
        return true;
    }
}
