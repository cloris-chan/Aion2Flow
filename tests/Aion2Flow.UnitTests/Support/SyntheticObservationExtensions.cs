...
internal static class SyntheticObservationExtensions
{
    ...
    public static void AppendNickname(this IRuntimeObservationSink sink, int uid, string nickname, int? originServerId = null, Faction faction = Faction.Unknown, CharacterClass? characterClass = null)
    {
        var source = Source();
        sink.AppendNickname(in source, uid, nickname, originServerId, faction, characterClass);
    }
    ...
    public static void RegisterObservation2C38(this IRuntimeObservationSink sink, int instanceId, int mode, int sequenceId, int resultCode, int tailSourceId, int tailSkillCodeRaw, long timestamp, long frameOrdinal, long batchOrdinal)
    {
        var source = new PacketObservationSource(timestamp, frameOrdinal, batchOrdinal, 0x2C38, 0, 0, default);
        sink.RegisterObservation2C38(in source, instanceId, mode, sequenceId, resultCode, tailSourceId, tailSkillCodeRaw);
    }
}
