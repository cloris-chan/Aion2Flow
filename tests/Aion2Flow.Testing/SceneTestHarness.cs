...
public sealed class SceneTestHarness : IDisposable
{
    ...
    private sealed class HarnessSink(SceneTestHarness owner, IRuntimeObservationSink inner) : IRuntimeObservationSink
    {
        ...
        public void RegisterCompactControl0238(in PacketObservationSource packet, int sourceId, int mode, uint bodyCodeRaw, int marker, int flag, int echoSourceId, int? availableCountAfterControl = null, int? cooldownMilliseconds = null) => inner.RegisterCompactControl0238(in packet, sourceId, mode, bodyCodeRaw, marker, flag, echoSourceId, availableCountAfterControl, cooldownMilliseconds);
        ...
        public void RegisterCooldownCharge2238(in PacketObservationSource packet, byte state, int packetSkillCode, int availableCount, int nextChargeRemainingMilliseconds) => inner.RegisterCooldownCharge2238(in packet, state, packetSkillCode, availableCount, nextChargeRemainingMilliseconds);
        public void RegisterCooldown4738(in PacketObservationSource packet, int rowBaseSkillId, int remainingMilliseconds) => inner.RegisterCooldown4738(in packet, rowBaseSkillId, remainingMilliseconds);
